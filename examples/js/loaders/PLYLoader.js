...
THREE.PLYLoader.prototype = {
	...
	parseHeader: function ( data ) {
		var patternHeader = this.isMicrosoft(data) 
					? /ply([\s\S]*)end_header\r\n/
					: /ply([\s\S]*)end_header\n/;
		var headerText = "";
		...
		var lines = this.isMicrosoft(data) ? headerText.split( '\r\n' ) : headerText.split('\n');
	...
	parseASCII: function ( data ) {
		...
		// PLY generated by Microsoft would use '\r\n' and 'property list uchar int vertex_index' {vertex_indicis}
		var geometry = new THREE.Geometry();

		var result;

		var header = this.parseHeader( data );
		var patternBody = this.isMicrosoft(data)
					? /end_header\r\n([\s\S]*)$/
					: /end_header\n([\s\S]*)$/;
		var body = "";
		...
		var lines = this.isMicrosoft(data) ? body.split( '\r\n' ) : body.split('\n');
	...
	handleElement: function ( geometry, elementName, element ) {
		...
		if ( elementName === "vertex" ) {
			...
			if(element.hasOwnProperty('vertex_index')){
				geometry.faces.push(
					new THREE.Face3( element.vertex_index[0], element.vertex_index[1], element.vertex_index[2] )
				)
			}else{
				geometry.faces.push(
